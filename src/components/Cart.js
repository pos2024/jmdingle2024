...
const decreaseQuantity = (id) => {
  ...
  return (
    <div className='bg-gradient-to-r from-[#623288] to-[#4B0082] flex flex-col justify-between h-screen w-2/5 p-4'>
          ...
          <ul className="mt-4">
            {cartItems.map((item) => (
              <li key={item.id} className="flex justify-between items-center mb-2">
                ...
                <span className="flex-grow px-2">{item.name}</span>
      ...
      <div className='bg-[#1d1d1d] p-2 rounded-md'>
        <div className="mt-8 text-white">
          <h2 className="text-xl font-bold">Total: ₱{total.toFixed(2)}</h2>
          <div className='flex space-x-2'>
          <h2 className="text-xl font-bold mt-2">Cash Given:<span>{cash}</span></h2>
          <button
              onClick={() => setCash(0)}
              className="bg-[#623288] text-white  text-sm font-bold py-1 px-2 rounded"
            >
              Clear
            </button>
            </div>
